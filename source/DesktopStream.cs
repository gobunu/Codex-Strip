...
namespace CodexStrip {
 ...
 public sealed class DesktopStream:IDisposable {
  ...
  readonly Dictionary<string,Card> readUpdates=new Dictionary<string,Card>();double desktopReadAt;
  string readIdentity="";
  ...
  public void RefreshReadState(){try{var path=Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile),".codex",".codex-global-state.json");double savedAt=(File.GetLastWriteTimeUtc(path)-new DateTime(1970,1,1,0,0,0,DateTimeKind.Utc)).TotalSeconds;object data;using(var f=new FileStream(path,FileMode.Open,FileAccess.Read,FileShare.ReadWrite|FileShare.Delete)){using(var r=new StreamReader(f))data=J.Parse(r.ReadToEnd());}bool changed;lock(gate){var next=ParseDesktopUnread(data,readIdentity);changed=desktopReadAt!=savedAt||J.Json(desktopUnread)!=J.Json(next);desktopUnread=next;desktopReadAt=savedAt;}if(changed)Changed();}catch{}}
  public void ObserveReadState(string host,string id,bool unread,double observedAt){lock(gate){string key=host+"/"+id;Card state;if(!readUpdates.TryGetValue(key,out state))readUpdates[key]=state=new Card{Host=host,Id=id};state.SetUnread(unread,observedAt);}Changed();}
  void ApplyReadStateLocked(Card card){HashSet<string> unread;if(desktopUnread.TryGetValue(card.Host,out unread))card.SetUnread(unread.Contains(card.Id),desktopReadAt);Card update;if(readUpdates.TryGetValue(card.Key,out update))card.MergeUnread(update);}
  // Read receipts apply to polled cards even when the conversation stream is stale or absent.
  public void ApplyReadState(IEnumerable<Card> cards){lock(gate){foreach(var card in cards)ApplyReadStateLocked(card);}}
  ...
  public Card[] Latest(){lock(gate){return states.Values.Where(s=>s.Preview!=null).Select(s=>{var c=s.Preview.Copy();ApplyReadStateLocked(c);c.Stale=!Connected||s.Invalid;return c;}).ToArray();}}
  ...
  async Task Handle(object msg){string type=J.Str(msg,"type"),method=J.Str(msg,"method");if(type=="client-discovery-request"){await Send(J.Obj("type","client-discovery-response","requestId",J.Str(msg,"requestId"),"response",J.Obj("canHandle",false)));return;}
   ...
   if(method=="thread-read-state-changed"&&J.Num(msg,"version")==3){if(host.Length>0&&id.Length>0&&J.Get(p,"hasUnreadTurn") is bool)ObserveReadState(host,id,(bool)J.Get(p,"hasUnreadTurn"),J.Now);return;}
 ...
 public sealed class StreamState {
  ...
  public static Card Project(object state,string host,string id){var card=new Card{Host=host,Id=id,Title=J.Str(state,"title"),MessageKind="实时对话",Live=true};
   ...
   // Conversation snapshots have no read-state revision. Dedicated receipts and
   // list_threads observations take precedence, even when this snapshot arrives later.
   if(J.Get(state,"hasUnreadTurn") is bool)card.SetUnread((bool)J.Get(state,"hasUnreadTurn"),0);
   ...
   var answered=new HashSet<string>();var answeredTexts=new HashSet<string>();
   foreach(var t in ordered.Where(t=>J.Str(t,"status")=="inProgress"))foreach(var item in J.Arr(J.Get(t,"items"))){string kind=J.Str(item,"type");if(kind!="userMessage"&&kind!="steeringUserMessage")continue;foreach(var part in J.Arr(J.Get(item,"input")).Concat(J.Arr(J.Get(item,"content")))){string text=J.Str(part,"text");const string open="<send_user_message_question_reply>",close="</send_user_message_question_reply>";int start=text.IndexOf(open,StringComparison.Ordinal),end=text.IndexOf(close,StringComparison.Ordinal);if(start<0||end<start)continue;try{foreach(var reply in J.Arr(J.Parse(text.Substring(start+open.Length,end-start-open.Length)))){var key=J.Arr(J.Parse(J.Str(reply,"questionItemId")));if(key.Length>=3)answered.Add(Convert.ToString(key[1])+"/"+Convert.ToString(key[2]));string answeredText=J.Str(reply,"question");if(answeredText.Length>0)answeredTexts.Add(answeredText.Trim());}}catch{}}}
   ...
   foreach(var request in J.Arr(J.Get(state,"requests"))){var p=J.Get(request,"params")??request;string kind=J.Str(request,"type")+" "+J.Str(request,"method");var questions=J.Arr(J.Get(p,"questions"));var unansweredQuestions=questions.Select(q=>J.Str(q,"question").Length>0?J.Str(q,"question"):J.Str(q,"title")).Where(q=>q.Length>0&&!answeredTexts.Contains(q.Trim())).ToArray();if(unansweredQuestions.Length>0&&ordered.Any(t=>J.Str(t,"status")=="inProgress")){card.QuestionPending=true;card.Message=string.Join("\n",unansweredQuestions);}else if(kind.IndexOf("approval",StringComparison.OrdinalIgnoreCase)>=0)card.Status="approval";}
