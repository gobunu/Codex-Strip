...
支持按屏幕分别记忆的 100%–200% 界面倍率（新屏幕默认 150%），以及顶部全屏按钮。按 F11 切换全屏，按 Esc 退出全屏；退出后恢复原来的浮动位置或屏幕停靠。

没有进行中的任务和待处理事项后，看板会在可设置的静息时间结束时切换到独立的资源监控页面。CPU、GPU、内存默认使用单环显示总占比与主要程序占比；网速和磁盘显示平均速率与实时双曲线趋势。点击页面即可返回任务。显示项目与顺序、圆环或条形样式、网卡、速率单位、颜色和背景图片可在设置中调整。
